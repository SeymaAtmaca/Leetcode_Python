...
class Solution:
    def candy(self, ratings: List[int]) -> int:
        n = len(ratings)
        candies = [1] * n # her çocuk en az bir şeker alıyor

        # sol komşuya göre kontrol yapılır
        for i in range(1,n):
            if ratings[i] > ratings[i-1]:
                candies[i] = candies[i-1] + 1 # rating i büyük olan komşularından fazla şeker alır

        # sağ komşuya göre kontrol yapılır
